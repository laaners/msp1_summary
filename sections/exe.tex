...
\pagebreak\subsection{20211108-2}
    ...
    \mybox{
        ...
        $$
        We know that the DFT of a sinusoid will introduce two symmetric pulses at $\pm\tilde{f}_0$, so in this case we will have 4 pulses. On the next equations we omitted the correct magnitude of each pulse, each term should be multiplied by 
        $$
        A=\underlabel{\frac{10}{2}}{In presence of cos, magnitude of pulse is $A/2$}=5
        $$
        $$
        X(f)=
    ...
    \mybox{
        ...
        \end{center}
        \textbf{The pulse at $\pi/3$ that introduces aliasing is due to symmetric component that is located AFTER the Nyquist frequency (not seen in the plot): IF ANY PULSE IS FOUND AFTER THE NYQUIST WE WILL HAVE ALIASING}
    }
